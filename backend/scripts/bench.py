...
def build_markdown(*, latency: dict, throughput: dict, accuracy: dict,
    ...
    lines.append("|---|--:|---|--:|")
    lines.append("| 白（只读查询） | 2 (+R) | ①选工具 ②据工具结果作答；(+R) 每条**不可信**工具输出多一次隔离阅读器 | ~12–24 s |")
    lines.append("| 灰（修改/动作） | 3 (+R) | ①**独立安全研判**(防线1.5) ②选工具 ③作答；(+R) 同上 | ~18–36 s |")
    ...
    lines.append("")
    lines.append("> **(+R) CaMeL 隔离阅读器的往返成本（P0-F）**：读到**不可信**工具输出（日志/进程/文件等带"
                 "`untrusted` 能力腿的工具）时，编排器会多发起一次**无工具**的隔离阅读器调用，把原始不可信字节"
                 "压成不可信摘要后才交规划器——这是「注入打不进控制流」的强制隔离的自觉代价。纯指标工具"
                 "（disk_usage/memory_info 等数值）无此开销。优化杠杆：阅读器可用更快/更小的本地模型，或对低敏感"
                 "只读输出按策略跳过隔离。")
    lines.append("")
