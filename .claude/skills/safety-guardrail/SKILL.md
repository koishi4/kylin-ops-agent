...
2. **权限类**：`chmod 777`、`chmod -R` 作用于系统目录、`chown` 改 root 属主
3. **磁盘类**：`mkfs.*`、`dd of=/dev/sd*`、重定向覆盖块设备 `> /dev/sd*`
4. **提权类**：未授权 `sudo`/`su`、修改 `/etc/sudoers`、改 root 密码
5. **注入类**：见防线3
6. **关键配置保护**：写入 `/etc/passwd /etc/shadow /etc/fstab sshd_config /etc/hosts` 等
