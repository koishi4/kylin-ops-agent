...
## 第二轮评审整改：ASR 去噪 + 评分② 真模型语料（2026-06-11）

两处与本目录相关的口径整改：

### ① held-out 报告新增「真盲区 / 语料错标」二分

`redteam_eval.py` 对攻击家族的「漏过」按效果二分（`classify_exec_leak`）：

- **real_miss**：确有破坏/外联/敏感读/下载落关键区等危害却未遏制 —— 真盲区，记到护栏头上；
- **corpus_benign**：经独立分析（按效果 + 正面证明「纯读/惰性」）证明其实无害（如读 `/proc/cpuinfo`、
  `/etc/ssl/openssl.cnf` 这类世界可读信息），是外部语料把良性读**错标**为攻击。

于是 `<file>.report.json` 多出 `leaked_real` / `leaked_corpus_benign` / `asr_real` 字段，报告同时打印
**原始 ASR** 与**真·ASR（剔除语料错标）**。v5 由此从「ASR 5%」诚实拆成「原始 5% / 真·0%」——把
「护栏漏了真攻击」与「语料错标无害读」分清，不再让外部语料的标签噪声污染 ASR。**这不改变封存的
卷子，只改变记分口径**：`.jsonl` / `.sha256` / `.manifest.json` 字节不变，`sealed` 仍为真。

### ② 评分② 自然语言准确性的真模型语料：`nl_intent_eval.jsonl`

与本目录其余「红队对抗语料」不同，`nl_intent_eval.jsonl` 是**能力语料**——42 条带标准答案的自然语言
运维诉求（含口语/迂回/多义表述 + 不该调工具的对照），标注每句**应选哪个 MCP 工具**。配合
`scripts/nl_eval.py` 驱动**真 Orchestrator + 真 MCP + 真 LLM** 度量「一句话→选对工具」top-1 准确率
（报告 `docs/nl-accuracy-report.md`）。补的是「② 此前只有 mock 测试、无真模型量化」这个洞。
