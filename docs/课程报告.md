...
1. 基于官方 MCP 协议封装 23 个只读运维感知工具，实现 OS 深度感知；
...
系统采用 B/S 分层架构，自上而下分为表示层（Vue3 五视图）、应用服务层（FastAPI 路由）、核心域层（编排器/护栏/动作/执行器/根因/审计）、工具层（MCP Server 23 只读工具）与外部依赖（DeepSeek、麒麟 OS、SQLite），如图 3.1 所示。
...
采用黑盒（功能用例）+ 白盒（规则覆盖、红队对抗、变形绕过）+ 第三方独立评测（NL2Bash 真实分布）+ 端到端（真模型）相结合。自动化测试共 541 个用例、55 个测试文件，全部通过。
