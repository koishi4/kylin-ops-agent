...
## 2026-06-11 第二轮独立评审整改：补②真模型基准 + 评测去噪 + 两处真误杀修复

> 一次客观的第三方代码评审（明确「不依赖项目文档、只看代码」）指出：核心护栏与红队方法论很扎实，
> 但**测试体系有三处「看着满分、实则口径不客观」**，且自测又翻出一个命令护栏盲区。本轮逐条解决。
> 价值观一以贯之：**发现并修补 > 宣称满分**；**让分数服从约束，而非让约束服从分数**。

### ① 命令护栏盲区：命令承载型包装器看穿内嵌销毁命令（自测发现）

`xargs`/`timeout`/`watch`/`flock` 与 sudo/env 这类「纯前缀」不同——它们先吃掉**自身旗标与定位实参**，
其后才是真正执行的命令。效果分析器按「首个非前缀词」取动词，于是内嵌的销毁动词被包装器名挡住，
**恰好落在 rm 正则覆盖不到的路径/动词时即静默放行**（实测 `allow=True`）：

| 命令 | 改前 | 改后 |
|---|---|---|
| `xargs shred /boot/vmlinuz` | **allowed** | DENY |
| `watch -n1 truncate -s 0 /etc/hostname` | **allowed** | DENY |
| `xargs -I{} rm -rf /root`（/root 不在 DEL-003 正则） | **allowed** | DENY |
| `flock -x /tmp/l rm -rf /root` / `timeout 5 xargs rm -rf /root`（嵌套） | **allowed** | DENY |
| `xargs ls -la` / `timeout 5 systemctl status` / `xargs rm -rf /tmp/build`（良性） | allowed | allowed（不误升）|

修复 `effect_analyzer._unwrap_carriers`：递归解到内嵌命令的 `(verb,args)` 再归集效果。安全方向不变——
解开只让内嵌效果**被看见**，是否升级仍由 `_critical_or_none`（落关键区才升）把关；解错至多归集不到效果、
退化为不升级，**绝不误杀**。新增 `test_guardrail_carrier_wrappers.py`（7 危险须拦 + 7 良性须放行）。

### ② 评分②「自然语言准确性」此前只有 mock 测试 → 补真模型量化基准

`test_nl_robustness` 跑在 `MockProvider`（关键词路由）上——测的是管道接没接对、以及 mock 自己的关键词表，
**与 deepseek 实际选不选对工具无关**。于是②（核心子项）在整个测试体系里**没有任何真模型数字**。补：

- `scripts/nl_eval.py` + `scripts/corpora/nl_intent_eval.jsonl`（42 条带标准答案、含口语/迂回/多义 +
  「不该调工具」对照），驱动**真 Orchestrator + 真 MCP + 真 LLM** 度量「一句话→选对工具」top-1。
- **关键方法学取舍**：工具 schema 取自**真实 MCP server**（与评委所见一致），但工具**执行**用即时只读桩
  （`_SchemaOnlyMCP`）——②量的是「选对工具」，工具真实运行时长与「选得对不对」无关（实测
  `find_large_files` 真扫全盘 >90s 会淹没「模型其实瞬间就选对」这一事实）。隔离的只是执行、不是选择。
- **结果（deepseek-v4-pro）**：首轮 **40/42=95.2%**，复跑 **42/42**；诚实区间记 **约 95–100%**——
  `nl-vuln-1` 的 miss↔hit 翻转源于**推理模型跨轮抖动**（vuln/posture 工具语义可互换），非任何修复。
  报告 `docs/nl-accuracy-report.md` 自带「跨轮稳定性」诚实区间声明。**单点不报「100%」**。
- **这个基准当场逼出两处真误杀**（见③④）——正是「测了真模型才暴露、mock 永远测不到」的价值。

### ③ 真误杀 A（防线4）：`\bpasswd\b` 命中**路径** /etc/passwd → `stat /etc/passwd` 被拦

privilege 的用户/口令管理正则按子串匹配，`/etc/passwd` 里的 "passwd" 让 `stat /etc/passwd`、
`cat /etc/passwd` 被误判「改口令、需 root」。改为**命令位锚定**（命令首位/分隔符后/sudo·su·doas 后）：
`passwd root`/`sudo passwd`/`usermod -aG` 仍识别，把 passwd 当**路径操作数**读取不再误伤。
新增 `test_privilege_path_fp.py`（7 须拦 + 7 须放行）。

### ④ 真误杀 B（防线1）：裸词「提权」把「检查有没有提权风险」判黑

意图分类器把 `提权`/`后门` 裸词无条件判黑，于是「帮我检查下有没有 Dirty Pipe 那类**提权**风险」
——正当防御排查——在入口被拦、到不了只读编排。改为**区分动作意图与诊断意图**：越权词在**非诊断语境**
（无 检查/排查/扫描/有没有/风险/漏洞/CVE… 标记）下才判黑。与既有「关闭防火墙 vs 防火墙开着吗」同思路。
动作意图（帮我提权/植入后门）仍判黑（test_nl_robustness 不回归）。新增 `test_classifier_privesc_diagnostic.py`。

### ⑤ 评测去噪 + 良性语料扩面：ASR 不再继承外部语料的标签噪声

- **良性对照 17→54 例**：补「只读关键区 / 非关键路径写删改权限 / 读关键区→写非关键的备份 / 下载落 tmp」
  这类「看着吓人实则常规」的运维。扩面后误拦率仍 **0%**（且正是它逼出了③的 `passwd` 真 bug）。
- **ASR 二分**：`classify_exec_leak` 把攻击家族「漏过」分为 `real_miss`（真盲区，记护栏头上）与
  `corpus_benign`（外部语料把世界可读的无害读错标为攻击，护栏正确未升级）。报告同时打印**原始 ASR**
  与**真·ASR（剔噪）**。v5 holdout 由此从「ASR 5%」诚实拆成「原始 5% / 真·0%」，两条
  `/proc/cpuinfo`·`openssl.cnf` 读明确标注语料错标。**不改卷子（sealed 不变），只改记分口径**。

### ⑥ 测量学诚实声明（写进 security-design §7）

把两条「叙事易被误读」处显式声明：① 命令护栏的路径裁决依赖**宿主机真实文件系统**（realpath 兜底的固有
代价，换机复现需同基线 FS）；② 自由形态 `execute(str)` 是**纵深防御层、非攻击者可达路径**——产品里 LLM
只能从只读工具里**选**、变更只走 3 个参数化动作，红队分数量的是「规则兜底层」而非端到端攻击面，**架构才是
第一道防御**。

**收尾**：全量 **2529 passed**；内置红队 88/88（真·ASR 0%、误拦 0%）；holdout v5 sealed=True 不变；
②基准 95–100%（snapshot）。本轮新增/修改均**测了真模型才暴露**的真问题，无一为「过某题」而打补丁。

## 2026-06-11 第三轮：意图层「去明文化」——移除破坏意图的关键词判黑（方向性纠偏）

**起因（用户质疑，且质疑得对）**：上一轮 ④ 给「提权」误杀打的补丁（「诊断 vs 动作」正则）本身就是
**黑名单跑步机**上的又一块胶布。用户进一步指出：现在几乎没有攻击者还用「格式化磁盘」这种明文去打
带过滤器的系统，而是用 **base64 编码 / 小语种（低资源语言越狱）/ 针对开源模型的 GCG 对抗性后缀**——
任何恶意意图都有无数种绕开明文过滤器的表述。**结论：意图层的明文关键词判黑，对真实攻击是摆设，
对正常用户却高频误杀，应当移除。**

**关键认识（为什么删了不丢安全）**：那个关键词**从来就不是**挡在攻击者与破坏之间的墙。本架构里
根本不存在「自然语言 → 任意命令执行」这条路——LLM 只能【选只读 MCP 工具】+【3 个强制二次确认的
白名单动作】。攻击者把 `mkfs` 用 base64+小语种+GCG 包到天上、把 LLM 彻底越狱，它手里也**没有能
格式化磁盘/提权的工具腿**，炸不了系统。关键词只是刷在混凝土墙上的涂料——拆掉涂料，墙还在。

**输入侧 vs 产物侧（这刀切准了再下）**：明文匹配在**输入侧**可绕；但 防线2 作用在**已被模型解码成的
具体命令产物**上——无论用户当初用英文/小语种/base64 问，落地的都是明文 `rm -rf /`，混淆到这一步早被
「编译掉」。所以「关键词匹配没意义」只对意图层成立，对 防线2 不成立。**别让对前者的正确批判，
连带删掉后者。**

**整改动作**：
- `classifier.py`：删除 `_BLACK_PATTERNS`（格式化磁盘/删整个系统/关安全设施）、`_PRIVESC_BACKDOOR`、
  `_DIAGNOSTIC_INTENT`（连带 ④ 的脆弱正则一起退场）。`classify_intent` 降格为**纯路由层**：
  白=只读快路 / 灰=进 防线1.5+命令级护栏 / 缺省保守判灰。**唯一保留 BLACK = 注入/操纵话术**，
  且明注「非安全承重，与 防线3 scan_injection 互为冗余」。模块 docstring 重写为设计立场声明。
- **测试改的是契约不是断言**（避免「靠删断言过测」）：
  - `test_classifier_privesc_diagnostic.py` 反转为**锁定新契约**——提权/后门表述（动作+诊断）一律
    **不得在意图层判黑**（防回归：别再把跑步机加回来）。
  - `test_nl_robustness.py`：危险组 A 拆成 **A1 注入（仍判黑）** + **A2 破坏意图（路由到灰，不硬拦）**。
  - `test_guardrail_redteam.py`：`TestIntentClassification` 的 BLACK 组改为 `DESTRUCTION_ROUTED_GRAY`，
    断言「路由到灰交下游确定性边界」。
  - `test_risk_assessor.py::test_rules_cannot_be_overruled_by_ai`：原靠**意图关键词** DENY 驱动
    「规则不被 AI 翻案」，现改为靠**命令级 防线2** DENY（`command="rm -rf /"`）驱动——这才是去明文化后
    规则侧确定性 DENY 的真实来源，property 不变、setup 更诚实。
- `security-design.md` 新增 **§7.4**：把这件事写成威胁建模卖点（明文过滤为何对 base64/小语种/GCG 是摆设，
  安全边界为何下沉到架构/防线1.5/防线2 三层确定性机制）。

**为什么不一并删注入判黑**：它误报低、有审计/演示价值，且 orchestrator 另有 `scan_injection` 独立兜底
（删了也不放过注入）——属保守保留，已在代码注释里诚实标注其「非承重」性质。
...
## 2026-06-11 隔离阅读器改「提取式」：救根因保真，不松隔离边界（评审 一.2 整改）

**触发**：第三方评审「一.2 CaMeL 双 LLM 隔离的信息损耗」——`tail_log`/`query_journal` 等 untrusted
工具走隔离阅读器（`_quarantined_read`），而 `READER_SYSTEM` 当时要求「简洁摘要、≤600 字」，**概括式**
摘要会把 dmesg 报错栈、nginx error 签名压没，导致下游根因分析（评分④）拿不到定位细节。

**判断**：评审的**保真担忧成立**（且恰好打在根因分析最依赖的日志工具上），但其建议的「对干净内容走
datamark-only 快路、绕过 LLM 阅读器」**不采纳**——那会让**原始不可信字节（即便 datamark）直达规划器**，
破坏本项目核心不变量「原始不可信字节绝不进规划器」（CaMeL 隔离的招牌创新）。用隔离换延迟，正是
CLAUDE.md 明令禁止的「为便利牺牲架构」。故只做**保真不破界**的那一半：

**整改动作**（`guardrail/flow_control.py`，不碰 `should_quarantine` 门控、不碰隔离路径）：
- `READER_SYSTEM` 由「概括」改「**提取**」：明确要求逐字保留报错签名/异常类名/错误码/失败单元、
  关键栈帧（文件名:行号、函数名）1–3 行、定位性数值（占用率/PID/路径/时间戳），只删噪声，
  禁止把具体报错压成「发生了一个错误」。**安全硬约束（无工具、不照做注入、脱敏凭据）原样保留且
  声明优先级最高**——保真服从隔离，不得越界。
- `MAX_SUMMARY_CHARS` 600 → **1200**：容得下异常签名 + 关键栈帧 + 数值，但仍是「受限提取」而非
  原始日志转储（典型日志 KB–MB 级，1200 字强制只提取关键行，抗外泄面仍在）。

**为什么不动延迟**：额外一次串行 LLM 往返是隔离不变量的固有代价、且只落在 untrusted 工具（纯指标
工具仍直喂、无往返）。用更快模型做阅读器是未来可选杠杆，但不值得为它引入新配置/新依赖，更不该靠
「绕过隔离」来省——隔离边界不卖。
...
## 2026-06-11 澄清「白名单可扩展、非 3 动作天花板」（评审 二.1 整改，文档纠偏）

**触发**：评审「二.1」称"全只读 MCP + 只有 truncate_log/kill_process/clean_path 三个动作 = 只读诊断
面板、不是运维 Agent，砍掉变更腿在生产会被拒收"。这是对架构的**误读**：架构禁止的是**自由形态 MCP
工具**，不是变更本身；动作白名单**本就可扩展**。

**判断**：这条**不是缺陷、是战略取舍 + 文档没讲透**，故只做文档纠偏（不改架构、不擅自加动作）：
- CLAUDE.md §4.0 增一段澄清：3 个动作是**当前实现集、非架构天花板**；扩实用性的正确方向是**往白名单
  加参数化动作**（`restart_service`/`reload_config`/`block_ip`…，各自「参数 schema + 二次确认 + 走
  executor 过防线2 + 审计」），能力随之增长而**不**给 LLM 自由 shell。Action-Selector（CaMeL）范式正是
  给 agent **安全变更能力**的业界解法；错误做法才是为"实用"放开任意执行。
- `系统可扩展性与未来工作.md` §6 已有「工具优先、命令兜底：高频动作做成 schema 约束的结构化工具、
  自由 shell 收敛为模板+AST+审批」的同向表述，无需重复。
...
- **前端补充（同日）**：给 `ConsoleView` 加「受控动作」抽屉面板——按类(服务/安全/清理/进程)列 7 个动作、
  动态参数表单 + 「授权(防线4)」开关 → **dry-run 预览护栏裁决** → 二次确认 → 经护栏执行 → 把动作的
...
## 2026-06-24 演示素材播种脚本 + 修「占位 OPERATOR_TOKEN 卡死受控动作演示」

**起因**：要在真机演示受控动作（评分③），需要「会真执行而非被拒」的垃圾文件/进程。动手前先核了一遍
落地条件，揪出一个**会让前端受控动作全 401 的部署坑**。

**坑（已修）**：`/action/execute` 挂 `require_operator`——token 为空=演示放行，非空=强制 Bearer 校验。
而 `.env.example` 出厂是 `OPERATOR_TOKEN=please-change-me`（**非空**），部署脚本回环分支又没动它 → 后端
**强制鉴权**；可随 git 下发的前端 `dist` 是用**空** `VITE_OPERATOR_TOKEN` 构建的、不带 token → 受控动作
必 **401**、演示当场失效。这个占位串是最坏值：既非空（不进演示模式）、又是公开已知（毫无安全性）。
修复：部署脚本回环分支把占位串**清空**（回到设计本意的「本机可信控制台演示模式」）；并告警 `--nginx`
会把 `/api` 暴露到本机 80（同网段可达，演示可接受，生产须配 token 并用同值重建前端 dist）。

**素材脚本 `scripts/seed_demo_junk.sh`**：一键造素材并打印演示清单（含 curl 兜底），关键是**保证落在
"会真执行"那一侧**——这几条边界是先用 `classify_file`+`check_command` 实测确认、写死进脚本的：
- `clean_path` 真删 → 目标放 `/tmp`（CLEANABLE 且不被命令护栏 PATH-001 拦）；放 `/var/log` 则**被 PATH-001
  拦**，正好演示「即便文件可清理，rm 落在 /var 仍被纵深防御拦死」。
- `truncate_log` 真清空 → `/var/log/*.log`（走进程内 `os.ftruncate`、不过命令护栏，CLEANABLE 即可）。
- `kill_process` 真杀 → 垃圾 `sleep` 进程**以服务账户 opsagent 起**（属主须等于后端身份，否则 kill EPERM
  执行失败）；文件同理 `chown opsagent`。这条「素材属主必须=后端运行身份」是真机演示最容易翻车的点。
- 评分④ → `fallocate` 造 150M 大日志，供「磁盘大文件定位 + 关键性判断」。
`--clean` 一键回收（按 `sleep 21600` 标记精确杀进程 + 删文件）。

**心智小结**：演示前一定要把「动作真能落地」的**每一层前置条件**走一遍（鉴权 token、文件属主=执行身份、
目标路径在护栏放行侧）。任一不满足，台上看到的就不是「安全执行」而是 401/EPERM/被拦——而且往往到现场才
发现。把这些边界**实测确认后写进播种脚本**，等于把演示的确定性固化下来。

## 2026-06-26 整理过时文档：历史规划单归档 + 活文档「3→7 动作」订正

**起因**：临提交盘点 `docs/`，发现两类「过时」：① 四份**已执行完的施工单**（`IMPROVEMENTS.md` /
`IMPROVEMENTS-v3` / `IMPROVEMENTS-v4` / `改进v2.md`）堆在顶层，与提交物混在一起；② **活文档内容与代码脱节**
——`actions.py` 的受控动作早已从 3 个扩到 **7 个**（+`restart_service`/`reload_config`/`block_ip`/
`clean_journal`），但 `security-design.md` 与 `CLAUDE.md §4.0` 仍写「只有 3 个动作」，是会被评委一眼看穿的硬伤。

**处理**：
- **归档而非删除**：四份施工单 `git mv` 进 `docs/archive/`，各加一行「已归档·勿据此施工」抬头，并建
  `docs/archive/README.md` 说明。为何不删——全仓约 14 处代码/测试注释按「P 编号」援引它们当设计依据
  （如 `guardrail/trifecta.py`→「改进v2 P3-2」、`api/routes.py`→「IMPROVEMENTS-v3 P0-4」）。这些引用**按名
  不按路径**，移目录不破坏其有效性，删除则让依据悬空。
- **订正活文档**：`security-design.md` 两处「3 个动作」、`CLAUDE.md §4.0` 图与澄清段的「三个/3 个动作」
  统一改为当前 7 个白名单动作；澄清段里原把 `restart_service`/`reload_config`/`block_ip` 当「未来可扩」的
  举例，改为「已落地」，未来举例换成尚未实现的 `rollback_config`/`quota_set`。
- 校验：活文档已无「当前时态」的 3 动作说法；无指向已移动文件的 markdown 断链。

**取舍小结**：「整理」不等于「删」。承载 why 的历史决策文档即便不再反映现状也有引用价值，归档+抬头标注
比删除更稳；真正该消灭的是**活文档里与代码脱节的事实**，那才是对外可信度的硬伤。

## 2026-06-26 代码注释软件工程学规范化：从「靠自觉」到「ruff 工具强制」

**起因**：要对全代码库（46 文件 / 8456 行）做注释规范化。先盘了现状，结论出乎意料——注释**已高度规范**：
Google 风格 docstring 100% 统一、零死代码、零 TODO/FIXME、风格无混杂，模块 docstring 覆盖 39/46（缺的 7 个
全是空 `__init__.py`）。所以这次的「规范化」不是补烂摊子，而是**把已有的好实践立成可执行、可 CI 的标准**
（与用户确认方向：立规矩+工具强制，而非给一堆所见即义的平凡函数硬塞样板 docstring 制造噪音）。

**做法**：
1. **建 `backend/pyproject.toml`**：只启用 ruff 的 pydocstyle（`D`）规则族，Google 约定——刻意不引入 E/F/I
   等与「注释」无关的检查，避免一次提交混入大面积无关改动（范围控制本身是工程纪律）。
2. **关键判断：禁用英文正字法规则**。首跑 386 violations，其中 **293 个是 D415**（首行须以 `.?!` 结尾）——
   纯属把中文 `。` 当「缺结束标点」的误报；D403（首词大写）同样是英文专属。给中文 docstring 硬加 ASCII 句号
   是错的，故在配置里显式 ignore D415/D403/D413 并写明理由。这一步把「噪音」与「真问题」分开：剩 82 个才是
   真正的结构/完整性缺口（缺 docstring 的类/方法、摘要后缺空行、Args 漏参、含反斜杠未用裸串）。
3. **补齐 82 个真缺口到清零**：7 个包 docstring + 17 个 dataclass 类 + 23 个 `to_dict`/property 方法 + 若干
   函数；把跨行摘要收成单行+空行修 D205；`_execute` 补全 4 个 Args（D417）；`_find_search_roots` 改裸字符串
   修 D301。生产代码 `ruff check app` → **All checks passed**。
4. **分层强制**：`app/` 压满 Google 风格；`tests/`、`scripts/` 只强制「有模块/包 docstring」（presence），
   放宽段落格式（D205/D209/D301）——测试/脚本 docstring 是自由式说明，不是对外 API 面，没必要套同等格式。
5. ruff 入 `requirements.txt`；标准写成 `docs/code-comment-style.md`；CLAUDE.md §6 接入「ruff check app 必须
   零违规、勿用 noqa 绕过」。

**验证**：`ruff check app tests scripts` 全绿；`pytest` **2647 passed**（注释改动零行为影响，含 `r"""` 化与
MCP 工具描述未受波及——只改了模块/类 docstring，工具函数自身 docstring 没动）。

**取舍小结**：对一个**已经规范**的库做「规范化」，价值不在「多写注释」而在「把隐性约定变成显性、可执行的
门禁」。两条判断最关键：① 工具默认规则不等于本项目的规范——**中文库要先把英文正字法规则关掉**，否则 76% 的
告警是噪音、淹没真问题；② 范围克制——只用 `D` 规则、只硬压 `app/`，不顺手开 E/F 把无关风格改动混进来。
立规矩的反面教材是「为过线而松规则 / 加 noqa」，那和护栏的黑名单跑步机一样是缓慢失守。

## 2026-06-26 SE 规范全面体检：ruff 强制集从 `D` 扩到 `D,F,I,B,UP`

**起因**：上一条把 docstring 立成 `D` 规则门禁时，刻意只开 `D`、不碰 `E/F/I`（为控范围、不让无关改动混进
同一提交）。用户接着要求「检查项目其他地方是否均符合 SE 规范」——这正是去把那条暂缓的范围补上：在 `D`
之外，代码本身的**正确性/整洁度**是否也过关。

**体检结果（ruff 诊断，全 `app/`）**：行宽 `E501` 875 处（几乎全是中文长注释），此外是**真缺陷**——
未使用 import `F401` ×3、过时 `typing` 导入 `UP035` ×5、导入未排序 `I001` ×6、异常未链 `B904` ×1、
冗余 `.encode("utf-8")`/`open(...,"r")`、带引号的注解 `UP037` 等。

**处置**：
1. **强制集 `D` → `D,F,I,B,UP`**：`F` 抓死代码/未定义名、`I` 统一导入序、`B` 抓易错模式、`UP` 现代化写法。
   这四族抓的是缺陷不是风格、无中文水土不服，故对 `app/` 与 `tests`/`scripts` **一视同仁**强制。
2. **仍不开 `E/W`（尤其 `E501` 行宽）**：理由同 `D415`——本库注释是中文长句，行宽规则只会逼着切断中文
   散文，噪声远大于价值。在 pyproject 注释里写明。
3. **`UP042`（`(str, Enum)`→`StrEnum`）显式 ignore**：它会改变 `str()` 语义（影响序列化/比较），属**行为
   变更**而非整洁化，不为「现代化」冒行为风险——这条判断和护栏「实用性扩展不卖任意执行」同一纪律。
4. **修复**：20 处 `--fix` 自动修（删 import、排序、`encode`/`open` 去冗余、去注解引号——均语义不变，且相关
   文件已有 `from __future__ import annotations`）；2 处 `B904` 手改——`rules.py` 正则编译失败 `raise ... from e`
   保留原因链、`demo.py` 把 EOF/Ctrl-C 归一为 `KeyboardInterrupt from None` 显式断链。

**验证**：`ruff check app tests scripts` 全绿；`pytest` **2647 passed**（逐行核对 diff：除 import 增删/排序外，
仅 `from e`/`from None` 与等价写法替换，零控制流改动）。

**取舍小结**：上一条「先只开 D」与这一条「再扩 F/I/B/UP」不是反复，而是**分两步、各自范围干净**的工程节奏——
先把高噪音的注释规范单独立稳，再以独立提交补足代码质量门禁，避免一锅烩。同一条红线两处复用：**能改善但有
行为风险的（`UP042`）就不强加**，宁可显式 ignore 并留档，也不让「规范化」夹带语义变化。

## 2026-07-06 运维简报（日报/周报）功能 + 前端设计系统整体重写

**背景**：项目将复用于《人工智能应用项目实训》方向一（自动化办公与效率工具 Agent）。方向一示例里
「周报自动生成器 / 推送每日待办简报」是评审词汇表的一部分——在不动安全架构的前提下补一个**锚点功能**：
运维简报生成。同时用户反馈网页前端「不够美观且 AI 味重」，要求整体重写。

### 一、运维简报（backend `core/briefing.py` + `GET /briefing`）

- **形态**：一键（period=daily/weekly）聚合四个既有 READONLY 数据源 → 结构化简报 + Markdown 渲染：
  ① psutil 系统快照；② `diagnosis.diagnose("all")` 健康诊断摘要；③ `posture.check_posture(live=False)`
  离线安全态势；④ `audit.store.activity_stats(since)`（新增：窗口内会话/拦截/污点/动作统计 + 拦截样例）。
- **纪律与项目安全架构同构**：全只读零副作用（建议仅供人工决策，处置走受控动作）；**确定性优先**——
  不依赖 LLM 也能出完整报告（mock/CI 可跑）；AI 导语是可选锦上添花：单次**无工具** LLM 调用、
  system prompt 明令「只许转述给定数据、禁止编造」，任何异常/空返回自动降级为确定性导语，绝不丢报告。
- **可追溯口径不破例**：每次生成落一条 `intent=briefing` 的审计 trace（聚合了哪些源、是否用 AI 导语、
  产出多大），简报本身可回放。
- 测试 `tests/test_briefing.py`（23 用例连同 audit 回归）：统计窗口口径、Markdown 六节齐全、AI 导语
  只在成功时启用、异常/空内容降级路径、API 422 拒未知 period、审计留痕。全绿，ruff 零违规。

### 二、前端整体重写（去组件库、去「AI 味」）

- **诊断旧版「AI 味」来源**：霓虹发光（box-shadow 光晕）、渐变底纹与渐变气泡、满屏 999px 胶囊圆角、
  emoji 语义符（⛔✓☣⚡）、Element Plus 默认组件形态。这些正是生成式模板的高频特征。
- **新设计语言「石墨 / 图纸 / 印章」**（theme.css 全量重写）：石墨底 + 发丝线分隔 + 方角（2~6px）；
  唯一品牌红是一枚**朱砂印章**（方块「麒」字 Logo，favicon 同步）；交互强调用低饱和图纸蓝；语义色
  全部降饱和（绿=放行、琥珀=需确认、红=拦截、五段链恒定阶段色不变）；等宽数字 + 编号式导航（01–07）
  + 全大写等宽微标签。无渐变、无发光、无胶囊、无 emoji。
- **移除 element-plus 依赖**：自持实现 Drawer/Toggle/toast/确认框（`ui.js` 单例 + App 宿主）、原生
  table/select/textarea/details。构建产物从 ~1.1MB 降到 **~210KB**（gzip ~77KB），vendor 只剩 vue。
- 七视图全部重写（对话/简报[新增]/体检/护栏/边界/审计/评委模式），功能与数据契约保持不变；
  对话流从「聊天气泡」改为「留言簿式」左标签 + 正文版式。
- **验证**：`npm run build` 通过；经 Chrome DevTools 真机渲染走查：对话（mock 选工具 + 10 段执行链
  回放）、简报生成、中文字体渲染均正常。

**取舍**：简报的 AI 导语坚持「LLM 只做改写、不做事实源」，与护栏「AI 只升不翻案」同一纪律——
报告失实比没有报告更糟；前端弃组件库不是炫技，而是「模板感」的根源治理，顺带把首屏体积砍掉 80%。

### 三、扫盘剪枝：伪文件系统 + /mnt Windows 挂载默认不深入（用户反馈）

真机走查时全盘简报在 WSL 下明显偏慢——`find_large_files(path=/)` 顺路走进了 `/mnt/c`（Windows 盘的
9p 网络挂载，跨 OS 遍历极慢且与「本机 Linux 磁盘诊断」无关）。修复（`_validate.py` + `disk.py`）：

- `windows_mounts()`：读 `/proc/mounts` 按 **fstype ∈ {9p, drvfs}** 判定 Windows 挂载——只认挂载表
  证据、不猜路径形态；非 WSL（麒麟/裸机）自然为空，零影响；挂载表不可读保守返回空（宁多扫不误剪）。
- `scan_prune_roots(scan_root)` + `prune_walk_dirs()`：os.walk 逐层剪枝。顺带修了一个既有盲区——
  `REFUSED_SCAN_ROOTS`（/proc /sys /dev /run）此前只拒「扫描根本身」，全盘扫描仍会走进 /proc；
  现在一并剪枝。**显式**以被排除目录为扫描根（如 `path=/mnt/c/data`）视为明确意图，不剪该前缀。
- 剪枝按字面路径分量判断（不逐目录 realpath）：这是性能/相关性优化而非安全边界，realpath 会把
  大目录树遍历拖慢一个量级。
