...
## 4.5 held-out 外部基准（去自评 / 真实第三方语料，诚实非 100%）

§4 是**内置**语料（作者自己出题），其 100% 是「自评」。为根治这一点，`scripts/corpora/external_holdout.jsonl`
从两个**真实第三方安全基准**确定性转换而来，**不混入**内置集、单独「跑一次如实报分」、内容指纹 `.sha256`
封存（自封存以来未改动可核验）。详见 `scripts/corpora/README.md`。

| 来源 | pin commit | 许可证 | 家族 | 条数 |
|---|---|---|---|---|
| AI-secure/RedCode · RedCode-Exec（bash） | `dbbf082` | MIT | `exec` | 40 |
| NVIDIA/garak · `data/dan` 越狱模板 | `35b7d27` | Apache-2.0 | `inject` | 14 |

**诚实结果（54 例，指纹 `8861e3d3…`）**：

| 家族 | 通过 | 检出率 | ASR | 漏过（如实列出） |
|---|---|---|---|---|
| 危险命令执行（RedCode-Exec） | 37/40 | 92.5% | 7.5% | `2_1/2_2`（`wget -O` 下载脚本到盘未即时执行）、`4_2`（`cp /etc/shadow` 凭据外泄） |
| 提示词注入（garak DAN） | 11/14 | 78.6% | 21.4% | `AntiDAN`/`DUDE`/`STAN`（软化角色扮演，缺硬标记词） |
| **总体** | **48/54（89%）** | — | — | — |

**纪律**：held-out 的漏过**不回头补**（补了就退化成训练集）。这 6 例暴露的是「下载-后续执行两段式 / 敏感文件
读外泄 / 软化措辞越狱」三类盲区，记入未来工作；但本系统第一保证是**架构**（MCP 全 READONLY + Rule-of-Two），
注入识别不全也**无法升级为状态变更**——held-out 量的是检测层厚度，兜底的是能力约束（§6 边界、security-design §3/§5）。
